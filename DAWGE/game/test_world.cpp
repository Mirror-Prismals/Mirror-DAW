...
//
// A stripped–down voxel demo that creates a superflat world consisting of a 24×24 grid
// of voxel cubes. Each cube is rendered with a grey stone base and a tight black grid
// overlay (as if its texture were 24×24 pixels).
//
...
const int GRID_DIM = 24;       // 24 x 24 grid
...
glm::vec3 cameraPos(GRID_DIM / 2.0f, 2.6f, GRID_DIM / 2.0f);
...
// In prone mode, we want the collision box to be 1 unit tall:
// We define:
//    getPlayerBoxMin() = (-0.3, -0.5, -0.3)
//    getPlayerBoxMax() = (0.3, 0.5, 0.3)
// so that feet = cameraPos.y - 0.5; hence if feet should be 1.0 then cameraPos.y must be 1.5.
...
glm::ivec3 raycastForBlock(bool /*place*/) {
    ...
    while (t < 10.0f) {
        ...
        if (p.y >= 0.0f && p.y <= 1.0f) {
            ...
            if (bx >= 0 && bx < GRID_DIM && bz >= 0 && bz < GRID_DIM)
...
// -------------------- Handle Collision --------------------
// If the player is within the grid area (x and z between 0 and GRID_DIM),
...
void handleCollision() {
    if (cameraPos.x >= 0.0f && cameraPos.x <= GRID_DIM &&
        cameraPos.z >= 0.0f && cameraPos.z <= GRID_DIM) {
...
// -------------------- Instance Data --------------------
std::vector<glm::vec3> createInstanceOffsets() {
...
// Fragment shader: renders a grey stone block with a tight black grid overlay.
...
int main() {
    ...
    std::vector<glm::vec3> instanceOffsets = createInstanceOffsets();
    glGenBuffers(1, &instanceVBO);
    glBindBuffer(GL_ARRAY_BUFFER, instanceVBO);
    glBufferData(GL_ARRAY_BUFFER, instanceOffsets.size() * sizeof(glm::vec3), &instanceOffsets[0], GL_STATIC_DRAW);
