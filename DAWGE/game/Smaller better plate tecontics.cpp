// Plate tectonics terrain with smaller, denser plates
#include <glad/glad.h>
...
GLuint compile(const char* v,const char* f){
    GLuint vs=glCreateShader(GL_VERTEX_SHADER), fs_=glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(vs,1,&v,NULL); glCompileShader(vs); int ok;
    glGetShaderiv(vs,GL_COMPILE_STATUS,&ok);
    if(!ok){char e[512]; glGetShaderInfoLog(vs,512,0,e); std::cout<<"VS: "<<e<<"\n";}
    glShaderSource(fs_,1,&f,NULL); glCompileShader(fs_); glGetShaderiv(fs_,GL_COMPILE_STATUS,&ok);
    if(!ok){char e[512]; glGetShaderInfoLog(fs_,512,0,e); std::cout<<"FS: "<<e<<"\n";}
    GLuint p=glCreateProgram(); glAttachShader(p,vs); glAttachShader(p,fs_); glLinkProgram(p);
    glDeleteShader(vs); glDeleteShader(fs_); return p;
}
...
int main(){
    ...
    GLFWwindow* win=glfwCreateWindow(SCRW,SCRH,"Plates Tiny",NULL,NULL);
    ...
    for(int i=0;i<50;i++){
        glm::vec2 s(rand()%100-50, rand()%100-50);
